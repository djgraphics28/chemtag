...
class QuestionController extends Controller
{
    public function downloadTemplate(): BinaryFileResponse
    {
        ...
        $levels = Level::orderBy('order')->get(['name']);
        ...
        $addRows([
            ...
            ['level_name', 'Name of the level — see LEVEL NAMES below. Copy exactly, including capitalisation.', 'Yes', ''],
        ...
        $addRows([[]]);
        $addRows([['LEVEL NAMES (copy exactly, including capitalisation)']], $heading);
    ...
    public function import(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($rows, $gameModes, $levels, $request, &$imported, &$skipped, &$errors, &$lineNumber, &$headerFound): void {
            foreach ($rows as $row) {
                $lineNumber++;
                ...
                [
                    $gameModeCode, $levelName, $promptText, $explanation,
                ...
                if (! isset($levels[$levelName])) {
                    $errors[] = "Row {$lineNumber}: unknown level_name \"{$levelName}\".";
                    $skipped++;
                ...
                $choices = array_values(
                    array_filter([$c1, $c2, $c3, $c4, $c5, $c6], fn ($c) => trim((string) $c) !== '')
                );
                ...
                if ($correctIndex < 0 || $correctIndex >= count($choices)) {
                    $errors[] = "Row {$lineNumber}: correct_choice \"{$correctChoice}\" is out of range (1-".count($choices).').';
                    $skipped++;
                ...
                foreach ($choices as $sortOrder => $choiceText) {
    ...
    {
        $questions = Question::with('gameMode:id,code,title', 'level:id,name', 'choices')
        ...
        return Inertia::render('admin/questions/index', [
            ...
            'filters' => $request->only('search', 'game_mode_id', 'level_id'),
        ]);
    }
